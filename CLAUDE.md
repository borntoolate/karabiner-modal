...
| **ゲームパッドの単押しは固定ベース。エンジニア業務のアプリは R2 層の 10 枠と左スティック押込だけ上書き**（`MOD_LOCKED` は不可）。机に向かうアプリ（`DESK_APPS`）だけ単押しも特化 | `gamepad/DESIGN.md` §5 |
| **ゲームパッドの ⌘N は R2 + 右スティック押込にグローバルで置く。** アプリ別にしない。⌥クリックは Claude Desktop の R2 + 左スティック押込だけ（Chrome ではダウンロードになる） | `gamepad/DESIGN.md` §5 |
