...
- **Claude Desktop 層とブラウザの追加、⌘N のグローバル化**（2026-09-20）。Claude Desktop は
  ...
  どのアプリでも ⌘N。**実機で一通り確認済み**（2026-09-20。`DESIGN.md` §9）
- **メニューバーと保存を足した**（2026-09-21）。R2 + B = ⌃F2（メニューバーにフォーカス。
  上書き不可の 5 枠め。⌘W はここを譲った）、R2 + 左スティック押込 = ⌘S（どのアプリでも。
  Claude Desktop だけ ⌥クリックのまま）。ターミナル層の ⌃C は R2 + B から R2 + Start へ
  （⌘K は外した）。F キーは Karabiner の作法どおり fn の有無で 2 本に展開する
  （`gen.py` の `manipulators()`）。**実機で確認済み**（2026-09-21。下記「やること 5」）
- **L2 は押している間 ⌃**（2026-09-21）。L / ハートと同じ lazy な修飾キーで、L2 + 十字 =
  Mission Control ⌃↑ / アプリケーション Exposé ⌃↓ / スペース ⌃← →。音声入力の単押しは
  `to_if_alone`（離した瞬間に送る）へ。**実機で確認済み**（2026-09-21。下記「やること 5」）
- **L 単押し = 左クリック、ナッジに expo、デッドゾーン 0.12**（2026-09-21）。押し込みで
  ポインタが跳ねる・微調整が粗い、への答え。狙うクリックは人差し指の L（スティックに
  触らない）、ドラッグは押込のまま。ナッジは 傾きの増分 × 300 × 傾き（浅いほど細かい）。
  設計は `DESIGN.md` §5「L 単押しは左クリック」/ §6「微調整」「押し込みで跳ねる問題」。
  **実機で確認済み**（2026-09-21。浅い傾きの微調整、L のクリック、押込で跳ねないこと、
  L + 十字の選択）
- ルール 13 本 / manipulator 176 件
...
- **R2 の上書きは 10 枠**（`MOD_DEFAULT` のうち `MOD_LOCKED` の 5 枠以外）。アプリを足すときは
...
|---|---|---|---|---|
| 左（カーソル） | **0.12** | **0.9** | **300**（expo 1） | 倒した量だけその場で動く。浅い傾きは細かく（0.03 で約 1.4 px）、深いほど大きく（端で 8 px）。デッドゾーンから倒し切って約 120 px。いっぱいに倒して 0.3 秒保持で動き続ける |
...
| 症状 | いじるもの |
|---|---|
| 軽く倒したときの動きが小さい / 大きい | `XY_SPEED["flick"]` 300 を上下 |
| 浅い傾きが細かすぎて遠くが遅い / 逆に粗い | `XY_SPEED["expo"]` 1 → 0.5（穏やか）/ 1.5（強く）。合計を揃えるなら flick も（0.5 なら 200、1.5 なら 400） |
| いっぱいに倒しても動き続けない | threshold 0.9 → 0.85。スティックが端まで届いていない |
| 保持したときの速さが合わない | `XY_SPEED["slow"]` 13 を上下 |
| 指を離してもカーソルが動く（ドリフト） | deadzone 0.12 → 0.2 |
| 押し込み（クリック）でポインタが跳ねる | 狙うクリックは L 単押しに。押込を使うなら deadzone 0.12 → 0.2 |
...
## やること 4 — Claude Desktop 層・ブラウザ層・⌘N（確認済み）

2026-09-20 に実機で一通り確認しました。サーフェスの切り替え（長押しで連打しない）、
サイドバー、新規セッション（R2 + A / 右スティック押込）、前後のセッション、右に新しい
セッション、サイドバーの項目への R2 + 左スティック押込（⌥クリック）での分割ビュー、
パレット、権限プロンプト（ハート + A / B）、ブラウザの ⌘L / ⌥⌘B、TextEdit / VS Code の ⌘N。
残っているのは机に向かうアプリで ⌘N のダイアログが B で閉じるかだけ（`DESIGN.md` §9）。
...
- ⌘⇧N「同じ設定で新規セッション」、⌘⇧⌫「セッションをアーカイブ」、⌘⇧D / ⌘J / ⌘⇧B の
  ペイン切り替え。どれも Web 層に定義がある。R2 + B はメニューバーで埋まった（2026-09-21）
  ので、枠を空けるなら R2 + 十字 ↓（⌃⌘\）か L / R（前後のセッション）から。メニューバー
  経由（R2 + B → Go / View メニュー）なら枠なしで届く

---

## やること 5 — メニューバー・保存・L2 の ⌃・L クリック（確認済み）

2026-09-21 に足して、同日に実機で一通り確認した。残っているのは R2 + B の fn 付きの
manipulator（「標準のファンクションキー」が OFF の Mac でしか発火しない）だけ。
表は何を送っているかの控え。

| 操作 | ボタン | Karabiner が出すもの | 期待 |
|---|---|---|---|
| ~~メニューバーにフォーカス~~ 確認済み | R2 + B（どのアプリでも。上書き不可） | `f2` + `left_control`（この Mac は「標準のファンクションキー」が ON なので fn なし。OFF なら `fn` 付きの別 manipulator が発火） | アプリ名のメニューが選ばれる。十字 ← → で移動、↓ / A で開く、A で実行、B で抜ける |
| ~~保存~~ 確認済み | R2 + 左スティック押込（Claude Desktop 以外） | `⌘S` | TextEdit / VS Code で保存。ブラウザでは「ページを保存」ダイアログが出るので B で閉じる |
| ~~強制中断~~ 確認済み | R2 + Start（ターミナルのグループ） | `⌃C` | 走っているコマンドが止まる |
| ~~Mission Control~~ 確認済み | L2 を握って十字 ↑ | `⌃↑`（L2 の lazy な `left_control` + 十字の `up_arrow`） | 全ウィンドウとスペースの一覧。左スティックで指して押込で選ぶ、B で抜ける |
| ~~アプリケーション Exposé~~ 確認済み | L2 を握って十字 ↓ | `⌃↓` | 前面アプリのウィンドウ一覧 |
| ~~スペースの移動~~ 確認済み | L2 を握って十字 ← → | `⌃← / ⌃→` | 隣のデスクトップへ |
| ~~音声入力（変わっていないか）~~ 確認済み | L2 単押し | `dictation`（**離した瞬間**に送る。`to_if_alone`） | これまでどおり開始 / 終了。1 秒以上押してから離すと何も起きない |
| ~~左クリック~~ 確認済み | L 単押し | `button1`（離した瞬間に。`to_if_alone`） | スティックが動かずにクリック。L 2 回でダブルクリック。L 押しっぱなしの ⇧ は変わらない |
| ~~微調整~~ 確認済み | 左スティックを浅く倒す | ナッジ = 増分 × 300 × 傾き | 0.03 の傾きで 1〜2 px。深く倒すと大きく。倒し切って約 120 px、保持で連続移動 |
| ~~押込の跳ね~~ 確認済み | 左スティック押込 | デッドゾーン 0.12 | 押し込んでもポインタが動かない（動くなら 0.2 へ） |

| 症状 | 見るところ |
|---|---|
| R2 + B で画面が明るくなる / 何も起きない | EventViewer で `f2` に化けているか。`defaults read -g com.apple.keyboard.fnState` が 1 なら fn なし、0 なら fn 付きが発火するはず。Karabiner 16.3.0 の変数 `system.use_fkeys_as_standard_function_keys` が期待どおりか |
| ⌃F2 は届くがメニューバーに移らない | システム設定 → キーボード → キーボードショートカット → キーボード の「メニューバーにフォーカスを移動」が ON か（既定は ON） |
| ⌘W が要る | ブラウザのタブは × をクリックするか、R2 + B → File → Close Tab。頻度が高ければブラウザ層の R2 + 左スティック押込に ⌘W を置く（`DESIGN.md` §5） |
| L2 + 十字 ↑ で Mission Control にならない | EventViewer で `up_arrow` に `control` が付いているか（lazy な ⌃ が十字の前に送られているか。ハート + 十字の ⌘矢印と同じ仕組み）。システム設定 → キーボードショートカット → Mission Control の ⌃↑ が ON か |
| L2 単押しで音声入力が始まらない | 離すまで押しっぱなしにしていないか（1 秒の `to_if_alone` 上限）。右スティックを回しながら離していないか（ホイールは alone を取り消す）。直らなければ `basic.to_if_alone_timeout_milliseconds` を L2 のルールに足して延ばす |
| 左スティックが全く動かない | 数式が通っていない。Karabiner-Elements → Devices → SN30 Pro → Game Pad の formula 欄にエラーが出ていないか。`pow` を `absolute_magnitude` の掛け算に書き換えて切り分ける |
| L に触れただけでクリックされて困る | `gen.py` の L のルールから `to_if_alone` を外す（`SHIFT_TAP`）。クリックは押込だけに戻る |
