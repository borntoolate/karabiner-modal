...
- **リーダーを押していないときの macOS を1ミリも変えない。** ⌘ も ⌃ も ⌥ も、
  Caps Lock から指を離せば完全に元のままです。
- リーダーを別のキーに変えるのが、この1つの manipulator の差し替えだけで済む。

### その代わりに生まれる問題

Caps Lock が修飾キーを出さないということは、**定義していない組み合わせは
どのルールにもマッチしない**ということです。マッチしないキーはアプリに素通りします。

そして素通りは「何も起きない」ではありません。**アプリと macOS が持っている
本来の割り当てが発火します。** 詳しくは §2b を参照してください。

---

## 2b. 決め打ちの送信と、押した修飾キーの転送は別物

この設定には、性質の違う2種類の出力があります。混ぜて考えると事故ります。

| | 例 | 出力の決まり方 |
|---|---|---|
| **決め打ちの送信** | `p` → ⌘V、`w` → ⌥→ | 生成時に固定。押し方に依存しない |
| **転送（パススルー）** | `⌥ + j` → ⌥↓ | **押した修飾キーに依存する** |

決め打ちの送信は最初からやっています。この設定は 100 個以上の `to` イベントで
修飾キーを送っています。「修飾キーを送らない」というルールは存在しません。

存在するのは次の3つで、それぞれ理由が別です。

1. **入力側で修飾キーを消費しない**（§2。Hyper 方式を採らなかった理由）
2. **押した修飾キーを出力に漏らさない**（§3。`optional: ["any"]` を避ける理由）
3. **素の Shift は転送しない**（Shift は Vim の大文字を表すため）

### 転送は覆っていない組み合わせを必ず残す

転送を1つ足すと「Caps + 修飾 + hjkl は矢印になる」という期待が生まれます。
しかし実際に覆えるのは列挙した組み合わせだけで、残りは素通りします。
**転送はカバー率を上げると同時に、残った穴の危険度を上げます。**

実際にこれで事故が起きました。`⌥` `⌘` を転送したあと、`Caps + ⌃ + k` を
Mission Control のつもりで押すと、素通りした `⌃k` が macOS のテキスト編集コマンドを
叩いて**行が消えました。**

### なぜ ⌥ と ⌘ は転送して、⌃ は転送しないのか

**判断材料は「修飾キーそのもの」ではなく「転送先での意味」です。**
転送ルールが出力するのは常に矢印キーなので、見るべきは修飾キー＋矢印の意味です。

| 組み合わせ | macOS での意味 | 転送 |
|---|---|---|
| `⌥` + 矢印 | 単語・段落の移動（テキスト） | する |
| `⌘` + 矢印 | 行頭行末・文書の先頭末尾（テキスト） | する |
| `⌃` + 矢印 | Mission Control / Spaces（テキストではない） | **しない** |

`⌃` が「テキストの修飾キーではない」わけではありません。むしろ逆で、
**`⌃` + 英字は macOS のテキストシステムそのもの**です（`⌃a` `⌃e` `⌃k` `⌃f` `⌃b`
などの Emacs 由来バインド）。この設定の Emacs 版が成立するのも、Vim 版が
`C-f` `C-b` を使えるのもそのおかげです。

macOS 自身が `⌃ + 英字` と `⌃ + 矢印` を別の体系に割り当てているので、
**矢印に転送する文脈に限って** `⌃` の行き先がない、というのが理由です。

---

## 2c. モードを閉じる（`from.any` による吸収）

Caps Lock を押している＝Vim の体系にいる、という前提を最後まで通します。
**定義していない `⌃` と `⌥` の組み合わせは、素通りさせずに飲み込みます。**

素通りさせたときに顔を出すのは、Vim とは別の2つの体系です。

| 修飾 | 素通りすると | Vim での同じキーの意味 |
|---|---|---|
| `⌃v` | 1ページ下（macOS） | 矩形ビジュアルモード |
| `⌃o` | 行を挿入（macOS） | ジャンプリストを戻る |
| `⌃y` | kill ring から貼り付け | 1行上にスクロール |
| `⌃a` | 行頭へ | 数値をインクリメント |
| `⌃k` | 行末まで削除 | ダイグラフ入力 |
| `⌥e` `⌥i` `⌥u` `⌥n` | **アクセントを予約して次の一打を食う** | （なし） |

`⌥` の方が危険です。デッドキーは画面上ほとんど変化がないまま次の打鍵を書き換えるので、
原因が追えません。

### 実装

`from.any` は個別の `key_code` の代わりに**すべてのキー**にマッチします。
公式ドキュメントに「キーを無効化する」用途の例がそのまま載っています。

```json
{
  "type": "basic",
  "description": "swallow every Control chord this config does not define",
  "from": {
    "any": "key_code",
...
`to` が空なのでキーは消えます。これが2本（`⌃` 系と `⌥` 系）で全体を閉じます。

**⌘ を含む組み合わせは意図的に閉じていません。** `mandatory` にも `optional` にも
`command` を書かないことがそのまま除外になります。⌘ を伴うチョードには
このルールが受け付けない修飾キーが余るのでマッチせず、アプリに素通りします。
⌘ はアプリのコマンド体系で、Vim の記法と衝突しません。`Caps + ⌘ + s` は保存のままです。

### 置き場所

**この2本は必ず最後に置きます。** `from.any` は全キーを覆うので、後ろに置いたルールは
永久に発火しません。`shared/validate.py` はこれを検出して `UNREACHABLE` を報告します。

### Emacs 版との違い

Emacs 版では `⌥` だけを吸収し、**`⌃` は素通りさせています。** macOS の `⌃` バインドは
Emacs バインドそのものなので、素通りしても設定の意味と一致するためです。
詳しくは `../emacs/DESIGN.md` を参照してください。

### まだ閉じていないもの

修飾なしと `⇧` だけの未定義キー（`Caps + i` が "i" を打つなど）は**そのままです。**
本来の Vim なら未定義キーは何も起きませんが、

- 矢印・Return・Delete・Tab は通す必要があり、`from.any` 一発では書けない
- `vim_mode` が立ちっぱなしで固まったとき、キーボードが完全に無反応になる

という2点があるため、しばらく使ってから判断することにしています。
...
「1行を選択 → ⌘X」を1打で実現しています。改行まで含めて選択されるので、
Vim の `dd` と同じく行そのものが消えます。

`yy` も同じ選択手順のあと ⌘C にし、最後に ← を送って選択を解除しています。

### 「1行を選択」は ⌘← と ⇧↓ ではない

素直に書けば「⌘← で行頭へ → ⇧↓ で次の行頭まで選択」の2手ですが、**これは壊れます。**

⇧↓ は**開始した桁を保ったまま**下の行へ動きます。⌘← が桁 0 に着地しなかった場合、
選択は次の行の同じ桁で終わり、次の行がそれより短ければ**その行の末尾**で終わります。
VS Code と JetBrains 系の ⌘← は最初の非空白文字で止まるので、インデントされた
コードでは選択が改行を越えて次の行を食います。

Monaco（VS Code のエディタ本体）で実測した結果です。

```css
.hoge {
  display: block;
  position: relative;   ← ここにキャレットを置いて ⌘← ⇧↓
}
```

選択されたのは `position: relative;\n}` でした。`dd` はここから ⌘X を送るので、
**閉じ括弧まで消えて、インデントの空白だけが残ります。**

そこで `gen.py` の `SELECT_LINE` は、**桁を一切当てにしない**5手に置き換えてあります。

| 手 | キー | 効果 |
|---|---|---|
| 1 | ⌘→ | 行末へ。曖昧さがなく、次の手を決定的にする |
| 2 | ⌘← | 行頭へ。賢いアプリはインデントの手前で止まる |
| 3 | ⌘← | もう一度。賢いアプリは桁 0 にトグルし、そうでないアプリでは何も起きない |
| 4 | ⌘⇧→ | 行末まで選択。桁の計算が入らない |
| 5 | ⇧→ | もう1文字ぶん＝改行。最終行では改行がないので何も起きない |

3手目のトグルは Monaco で実測（⌘← が「非空白文字 → 桁 0 → 非空白文字」と往復する）、
何も起きないほうは Chromium の `<textarea>` で実測しています。

**最悪ケースでも次の行は無傷です。** 行頭が賢いのにトグルしないアプリがあれば、
インデントの空白が消し残りますが、次の行の中身には手が届きません。
`SELECT_LINE` は `dd` `yy` `⇧v`（`V`）の3か所で共有しています。
...
{ "from": { "key_code": "v", "modifiers": { "mandatory": ["shift"] } },
  "to": [
    ...SELECT_LINE...,                                       // 1行を選択（§4 参照）
    { "set_variable": { "name": "vim_visual", "value": 1 } }
  ] }
```

このあと `j` を押すと `⇧↓` が足されて、さらに1行ずつ伸びていきます。
`SELECT_LINE` の選択は次の行の桁 0 で終わっているので、`j` を足しても
桁がずれず、行単位のまま伸びます。

### 操作キー

ビジュアルモード中は `y` `d` `x` `c` `p` が専用の動作（コピー／切り取り／削除／置換）に
...
**ここは意図的に Vim と違います。** Vim の `x` はビジュアルモードでは `d` と同じで、
消したものはレジスタに残ります。Vim ではそれで損をしません。無名レジスタと
システムのクリップボードは別の箱だからです。**macOS には箱が1つしかありません。**

削除をすべて ⌘X にすると、「貼り付けたい内容をコピー → 貼り付け先を選択 →
消す → 貼る」という日常的な手順が、3手目でクリップボードを失って破綻します。

そこで2つの意味にキーを1つずつ割り当てています。

| キー | 送るもの | 意味 |
|---|---|---|
| `d` `c` | ⌘X | 切り取り。Vim の契約どおり `p` で貼り戻せる |
| `x` | Delete | 削除。クリップボードに触れない |

ビジュアルモード外の `x` がもともと単なる前方削除（クリップボードを経由しない）
なので、キーの意味としては一貫しています。行ごと消したいときは `⇧v` → `x` です。
